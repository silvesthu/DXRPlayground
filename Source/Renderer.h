...
struct Renderer
{
	struct Runtime : RuntimeBase<Runtime>
	{
		...
		Texture									mShapeNoise3DTexture		= Texture().Width(128).Height(128).Depth(128).Format(DXGI_FORMAT_R8G8B8A8_UNORM).SRVIndex(ViewDescriptorIndex::ShapeNoise3DSRV).Name("Renderer.ShapeNoise3D").Path(L"Asset/TileableVolumeNoise/noiseShapePacked.dds");
		// Texture									mErosionNoise3DTexture		= Texture().Width(32).Height(32).Depth(32).Format(DXGI_FORMAT_R8G8B8A8_UNORM).SRVIndex(ViewDescriptorIndex::ErosionNoise3DSRV).Name("Renderer.ErosionNoise3D").Path(L"Asset/TileableVolumeNoise/noiseErosionPacked.dds");
