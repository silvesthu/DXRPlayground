	...
	for (;;)
	{
		...
		if (query.CommittedStatus() == COMMITTED_TRIANGLE_HIT)
		{
			...
			// HitConext is constructed, fill the debug values
			{
				...
				DebugValue(PixelDebugMode::PositionWS_InstanceID, path_context.mRecursionCount, float4(hit_context.HitPositionWS(), hit_context.mInstanceID));
			}
			...
			if (hit_context.BSDF() == BSDF::Light)
			{
				...
				else if (mConstants.mSampleMode == SampleMode::MIS)
				{
					...
					DebugValue(PixelDebugMode::MIS_BSDF, path_context.mRecursionCount - 1 /* for prev BSDF hit */, float4(path_context.mPrevBSDFSamplePDF, light_pdf, mis_weight, 0));
				}
			}
			else
			{
					...
					)
				{
					...
					float light_sample_direction_pdf					= 0;
					float3 light_sample_direction						= 0;
					LightEvaluation::GenerateSamplingDirection(light, hit_context.HitPositionWS(), path_context, light_sample_direction, light_sample_direction_pdf);
					
					float light_pdf										= light_sample_direction_pdf * LightEvaluation::SelectLightPDF(light_index);
					if (light_pdf > 0)
					{
						RayDesc shadow_ray;
						shadow_ray.Origin								= hit_context.HitPositionWS();
						...
						if (shadow_query.CommittedStatus() == COMMITTED_TRIANGLE_HIT && shadow_query.CommittedInstanceID() == light.mInstanceID)
						{
							...
							DebugValue(PixelDebugMode::Light_BSDF, path_context.mRecursionCount, float4(bsdf_context.mBSDF, 0));
							DebugValue(PixelDebugMode::Light_PDF, path_context.mRecursionCount, float4(light_pdf, 0, 0, 0));
							...
							{
								float mis_weight						= max(0.0, MIS::PowerHeuristic(1, light_pdf, 1, bsdf_context.mBSDFSamplePDF));
								light_emission							*= mis_weight;
								
								DebugValue(PixelDebugMode::MIS_Light, path_context.mRecursionCount, float4(bsdf_context.mBSDFSamplePDF, light_pdf, mis_weight, 0));
							}
				...
				// Sample BSDF
				{
					...
					path_context.mThroughput							*= bsdf_context.mBSDFSamplePDF > 0 ? (bsdf_context.mBSDF * abs(bsdf_context.mNdotL) / bsdf_context.mBSDFSamplePDF) : 0;
					path_context.mEtaScale								*= bsdf_context.mEta;
					
					path_context.mPrevBSDFSamplePDF						= bsdf_context.mBSDFSamplePDF;
					path_context.mPrevDiracDeltaDistribution			= hit_context.DiracDeltaDistribution();

					DebugValue(PixelDebugMode::BSDF__BSDF,				path_context.mRecursionCount, float4(bsdf_context.mBSDF, 0));
					DebugValue(PixelDebugMode::BSDF__PDF,				path_context.mRecursionCount, float4(bsdf_context.mBSDFSamplePDF, 0, 0, 0));
					...
					// Go for next bounce
					ray.Origin = hit_context.HitPositionWS();
			...
			case DebugMode::Position: 					path_context.mEmission = hit_context.HitPositionWS(); continue_bounce = false; break;
		...
		if (path_context.mRecursionCount >= mConstants.mRecursionCountMax)
		{
			...
			if (mConstants.mRecursionMode == RecursionMode::RussianRoulette)
			{
				...
				DebugValue(PixelDebugMode::RussianRoulette, path_context.mRecursionCount, float4(probability_passed, probablity, continue_probablity, 0));
				DebugValue(PixelDebugMode::EtaScale, path_context.mRecursionCount, float4(path_context.mEtaScale, 0, 0, 0));				
