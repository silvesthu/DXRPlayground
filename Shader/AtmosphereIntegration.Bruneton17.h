...
float Encode_R_Transmittance(float r)
{
	...
	float R_t = mConstants.mAtmosphere.mTopRadius;						// Top
...
float2 Decode2D_Irradiance(float2 u_mu_u_r)
{
	...
	float r = lerp(mConstants.mAtmosphere.mBottomRadius, mConstants.mAtmosphere.mTopRadius, u_mu_u_r.y);
...
float2 Encode2D_Irradiance(float2 mu_r)
{
	...
	float u_r = invlerp(mConstants.mAtmosphere.mBottomRadius, mConstants.mAtmosphere.mTopRadius, mu_r.y);
...
float4 Decode4D(uint3 inTexCoords, RWTexture3D<float4> inTexture, out bool intersects_ground)
{
	...
	if (true)
	{
		...
		uint slice_count = mConstants.mAtmosphere.mSliceCount;
	...
	// mu_s - Sun Zenith
	{
		...
		switch (mConstants.mAtmosphere.mMuSEncodingMode)
		{
...
void Encode4D(float4 r_mu_mu_s_nu, bool intersects_ground, Texture3D<float4> inTexture, out float3 outUVW0, out float3 outUVW1, out float outS)
{
	...
	// mu - View Zenith
	float r_mu = r * mu;
	float discriminant = r_mu * r_mu - r * r + mConstants.mAtmosphere.mBottomRadius * mConstants.mAtmosphere.mBottomRadius;
	float u_mu;
	if (intersects_ground)
	{
	...
	case AtmosphereMuSEncodingMode::Bruneton17:
	{
		...
		float d = DistanceToTopAtmosphereBoundary(mConstants.mAtmosphere.mBottomRadius, mu_s);
		float d_min = mConstants.mAtmosphere.mTopRadius - mConstants.mAtmosphere.mBottomRadius;
		float d_max = H;
		float a = (d - d_min) / (d_max - d_min);
		float D = DistanceToTopAtmosphereBoundary(mConstants.mAtmosphere.mBottomRadius, mu_s_min);
		float A = (D - d_min) / (d_max - d_min);
...
float IntegrateDensity(DensityProfile inProfile, float2 mu_r)
{
...
#endif // BRUNETON17_ADJUST_INTEGRATION
	{
		...
		float altitude = r_i - mConstants.mAtmosphere.mBottomRadius;
...
float3 ComputeTransmittance(float2 mu_r)
{
	...
	float3 rayleigh		= mConstants.mAtmosphere.mRayleighExtinction.xyz * IntegrateDensity(mConstants.mAtmosphere.mRayleighDensity, mu_r);
	float3 mie			= mConstants.mAtmosphere.mMieExtinction.xyz *		IntegrateDensity(mConstants.mAtmosphere.mMieDensity, mu_r);
	float3 ozone		= mConstants.mAtmosphere.mOzoneExtinction.xyz *	IntegrateDensity(mConstants.mAtmosphere.mOzoneDensity, mu_r);
...
float3 GetTransmittanceToSun(float r, float mu_s)
{
	...
	float sin_theta_h = mConstants.mAtmosphere.mBottomRadius / r;
	float cos_theta_h = -sqrt(max(1.0 - sin_theta_h * sin_theta_h, 0.0));

	// [Bruneton08 Doc] Approximate visible sun disk fraction
	// [TODO] Add notes
	return GetTransmittanceToTopAtmosphereBoundary(r, mu_s) *
		smoothstep(-sin_theta_h * mConstants.mAtmosphere.mSunAngularRadius, sin_theta_h * mConstants.mAtmosphere.mSunAngularRadius, mu_s - cos_theta_h);
}
...
void ComputeSingleScatteringIntegrand(float4 r_mu_mu_s_nu, float d, bool intersects_ground, out float3 rayleigh, out float3 mie)
{
	...
	rayleigh = transmittance * GetProfileDensity(mConstants.mAtmosphere.mRayleighDensity, r_d - mConstants.mAtmosphere.mBottomRadius);
	...
	uint inGroupIndex : SV_GroupIndex)
{
	...
	float alpha_s = mConstants.mAtmosphere.mSunAngularRadius;
...
void IntegrateSingleScattering(float4 r_mu_mu_s_nu, bool intersects_ground, out float3 rayleigh, out float3 mie)
{
	...
	rayleigh = rayleigh_sum * dx * mConstants.mAtmosphere.mRayleighScattering;
...
float3 ComputeScatteringDensity(float4 r_mu_mu_s_nu, bool intersects_ground)
{
	...
	for (int l = 0; l < SAMPLE_COUNT; ++l)
	{
		...
		if (ray_r_theta_intersects_ground)
		{
			...
			ground_albedo = mConstants.mAtmosphere.mGroundAlbedo;
		}
		...
		for (int m = 0; m < 2 * SAMPLE_COUNT; ++m)
		{
			...
			uint scattering_order = mRootConstants.mData0.x;

			// The radiance L_i arriving from direction omega_i after n-1 bounces is
			// the sum of a term given by the precomputed scattering texture for the
			// (n-1)-th order:
			float nu1 = dot(omega_s, omega_i);
			float3 incident_radiance = GetScattering(r, omega_i.z, mu_s, nu1, ray_r_theta_intersects_ground, scattering_order - 1);
			...
			float3 ground_irradiance = GetIrradiance(mConstants.mAtmosphere.mBottomRadius, dot(ground_normal, omega_s));
			...
			float rayleigh_density = GetProfileDensity(mConstants.mAtmosphere.mRayleighDensity, r - mConstants.mAtmosphere.mBottomRadius);
			float mie_density = GetProfileDensity(mConstants.mAtmosphere.mMieDensity, r - mConstants.mAtmosphere.mBottomRadius);

			rayleigh_mie += incident_radiance *
				(
					mConstants.mAtmosphere.mRayleighScattering * rayleigh_density * RayleighPhaseFunction(nu2)
					+
					mConstants.mAtmosphere.mMieScattering * mie_density * MiePhaseFunction(mConstants.mAtmosphere.mMiePhaseFunctionG, nu2)
					) * domega_i;
		}
	}
...
float3 GetExtrapolatedSingleMieScattering(float4 scattering)
{
	...
	return scattering.rgb * scattering.a / scattering.r * (mConstants.mAtmosphere.mRayleighScattering.r / mConstants.mAtmosphere.mMieScattering.r) * (mConstants.mAtmosphere.mMieScattering / mConstants.mAtmosphere.mRayleighScattering);
}
...
void GetSunAndSkyIrradiance(float3 inPositionPS, float3 inNormal, out float3 outSunIrradiance, out float3 outSkyIrradiance)
{
	...
	outSkyIrradiance *= mConstants.mAtmosphere.mSolarIrradiance;

	// Direct irradiance from sun
	outSunIrradiance = GetTransmittanceToSun(r, mu_s) * max(dot(inNormal, sun_direction), 0.0);
	outSunIrradiance *= mConstants.mAtmosphere.mSolarIrradiance;
}
