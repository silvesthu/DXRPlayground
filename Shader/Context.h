...
struct MediumContext
{
	...
	void ApplySequence(inout PathContext ioPathContext)
	{
		...
		float noise_value_raw					= ErosionNoise3D.SampleLevel(BilinearWrapSampler, (PositionWS() + offset) * 1.0, 0);
		float noise_value						= saturate(pow(noise_value_raw, 16.0));

		float y_animation_ratio					= pow(saturate(1.0 - ratio - 0.1), 0.5f);
		float y_animation						= saturate(remap(PositionWS().y - noise_value_raw * 0.2 + 0.2, 1.0f - y_animation_ratio, 1.0f + lerp(0.0f, 0.2f, ratio) - y_animation_ratio, 0.0f, 1.0f));
